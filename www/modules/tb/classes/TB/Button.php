...
class TB_Button extends TB
{
	...
	private static function storeButton($type, $value, $attributes, $hasDropdown)
	{
		...
		$currentInstance = self::$instance ? : new TB_Button;
	...
	public function with_icon($icon, $attributes = array(), $prependIcon = TRUE)
	{
		...
		$icon = TB_Icon::make($icon, $attributes);
	...
	public function deemphasize()
	{
		...
		$this->currentButton['attributes'] = TB::add_class($this->currentButton['attributes'], 'btn-link');
	...
	{
		// Add class to attributes array
		$this->currentButton['attributes'] = TB::add_class($this->currentButton['attributes'], 'btn-block');
	...
	public static function __callStatic($method, $parameters)
	{
		...
		if (count($type_found) > 0)
		{
			...
			$parameters = TB::set_multi_class_attributes($function, $method_array, $parameters, $attr_index, 'btn-', 'disabled');
	...
	public function __toString()
	{
		...
		$attributes = TB::add_class($attributes, 'btn');
		...
		if ($hasDropdown)
		{
			...
			$attributes = TB::add_class($attributes, 'dropdown-toggle');
