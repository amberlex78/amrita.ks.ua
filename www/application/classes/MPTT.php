...
/**
 * Modified Preorder Tree Traversal (MPTT) class.
 *
...
class MPTT extends ORM
{
	 ...
	 *
	 * @param Validation $validation
	 * @param null       $scope       the new scope
	 *
	 * @return $this|bool
	 * @throws Exception|ORM_Validation_Exception
	 */
	 ...
	 * Insert the object
	 *
	 * @param ORM_MPTT|int  $target          primary key value or ORM_MPTT object of target node.
	 ...
	 * @throws Exception|Kohana_Exception
	 */
	 ...
	 *
	 * @param bool $root   with root node
	 * @param null $scope  only retrieve nodes with specified scope
	 *
	 * @return Database_Result
	 */
	public function fulltree($root = TRUE, $scope = NULL)
	{
		$result = self::factory($this->object_name());

		if ( ! $root)
			$result->where($this->_primary_key, '<>', $this->pk());
	...
	/**
	 * Return first child of the current node.
	 *
	 * @return ORM_MPTT
	 */
	...
	/**
	 * Return last child of the current node.
	 *
	 * @return ORM_MPTT
	 */
	 ...
	 * @param   bool|int  number of results to get
	 * @return  ORM_MPTT
	 */
	...
	/**
	 * Return array ids descendants of the current node.
	 *
	 * @param bool $self
	 *
	 * @return mixed
	 */
	public function descendants_ids($self = FALSE)
	{
		$left_operator  = $self ? '>=' : '>';
		$right_operator = $self ? '<=' : '<';
	 ...
	 *
	 * @param str   $key      first table column.
	 * @param str   $val      second table column.
	 * @param null  $indent   character used for indenting.
	 * @param array $unshift  add an empty value to the start of a select list.
	 *
	 * @return array
	 */
	public function mptt_for_select($key, $val, $indent = NULL, array $unshift = array())
	{
		$result = DB::select($key, $val, $this->level_column)
			->from($this->_table_name)
		...
		if (is_string($indent))
		{
			...
			{
				$array[$row[$key]] = str_repeat($indent, $row[$this->level_column] - 2) . $row[$val];
			}

			return $array;
		}

		$result = $result->as_array($key, $val);

		if (isset($unshift[0]) AND isset($unshift[1]))
		{
			Arr::unshift($result, $unshift[0], $unshift[1]);
		}
	...
	/**
	 * Change enabled status and return changed array of ids
	 *
	 * @param bool $force_val
	 *
	 * @return array
	 * @throws Exception|Kohana_Exception
	 */
	public function mptt_change_enabled($force_val = NULL)
	{
		...
		if ($force_val === NULL)
		{
			...
			// Do not allowed enable status if parent enabled = 0
			if ( ! $p->enabled)
				return array();
		}
		else
